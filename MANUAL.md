...
> 適用版本：**v0.5**（v0.3 債權申報與彙整流程；**無還款管理**；含債權彙整表／6 類債權／對內債權，並新增**填報防呆、一類一筆、參與行治理防護、案件列表篩選／排序／分頁、草稿異動刪除與調解庭期、個人化 Dashboard**）。本手冊說明**各角色如何操作**與**如何測試**，並附流程圖。
...
**T18 Dashboard：待辦、期限與 KPI 深連結（v0.5）**
1. 以**受邀但尚未填報**的行登入 → ✅ Dashboard 最上方「需要您處理」出現該案，標記「**待我填報**」，按鈕為「填報本行債權」，點擊直達案件頁。
2. 填完明細但**先不確認**，回 Dashboard 重新整理 → ✅ 同一案的標記改為「**待我確認**」，按鈕變「確認無誤」。
3. 確認後重新整理 → ✅ 該案**自佇列消失**；若全員皆已確認，主辦端出現「**待我回報**」、其他參與行出現「**揭露後待檢視**」。
4. 主辦開一件**草稿**案件 → ✅ 佇列出現「**待發布**」。
5. 待處理超過 6 筆時 → ✅ 只列前 6 筆，右上出現「查看全部 N 筆待處理 →」。
6. 把某案調解庭期改為**未來 3 天內** → ✅「期限將至」出現該案、顯示「還剩 N 天」且**標紅**；改為 3 天以上 → ✅ 恢復一般樣式；超過 14 天者**不列入**。
7. 點「我主辦的案件」的「**本行待填報**」 → ✅ 跳至案件列表且已套用「主辦＋本行待確認」篩選；點「封閉申報中」→ ✅ 套用對應狀態篩選。**按 F5 篩選仍在**（條件寫在網址）。
8. 「最近動態」 → ✅ 只出現與本行參與案件相關的事件，未讀者前方有藍點，可「前往案件 →」。
9. **沒有任何待辦時** → ✅ 顯示空狀態「目前沒有待您處理的事項」，不是空白區塊。
10. 頁面**不會自動更新**（刻意設計）；請按右上「**重新整理**」或 F5。
11. 以**平台稽核**登入 → ✅ 見全平台各狀態案件、本月監督指標、**疑義熱點**（被退回 ≥1 輪者）、稽核事件流。
12. 以**平台管理員**登入 → ✅ 見「平台全盲」提示、待啟用帳號／待重置申請、各狀態案件**僅件數**、機構起案活躍度、最近治理事件；**全頁不出現任何金額**。
...
| 下拉找不到某銀行／法院 | 該機構**未啟用**；請管理員於「機構啟用管理」開啟。**法院目前已開放全國 22 個地方法院**（少年及家事法院不開放——消債前置調解屬地方法院管轄）；銀行則依試辦範圍逐步啟用。 |
...
| Dashboard 數字沒更新 | **刻意不自動輪詢**；請按右上「重新整理」或 F5。 |
| Dashboard「需要您處理」是空的 | 表示目前沒有輪到貴行的動作。已確認、已揭露待他行處理的案件不會列入；仍可從案件列表查看全部。 |
| 未讀通知數與待處理筆數不一致 | 兩者衡量不同事：鈴鐺是**未讀通知則數**，佇列是**尚未完成的本行動作件數**。讀過通知不會讓待辦消失，反之亦然。 |
